...
const App = () => {
  ...
  useEffect( () => {
    onAuthStateChanged(auth, async (user) => {
      if(user) {
        const data = await getUser(user.uid);   //this is where GET request to our server takes place and user data is fetched from mongodb
        dispatch(userExist(data.user));         //this is where we store the user data into redux state
      }
    ...
    <Router>
      ...
      <Suspense fallback={<Loader />}>
        <Routes>
          ...
          <Route
            element={
              <ProtectedRoute isAuthenticated={user ? true:false} adminRoute={true} isAdmin={user && user.role === "admin" ? true:false} />
            }
          >
